...
SELECT
    ...
    CAST(
        il.quantity * il.unit_price AS DECIMAL(12, 2)
    ) AS line_amount
...
SELECT
    ...
    SUM(CASE
        WHEN invoice_line_id IS NULL THEN 1
        ELSE 0
    END) AS missing_invoice_line_ids,

    SUM(CASE
        WHEN invoice_id IS NULL THEN 1
        ELSE 0
    END) AS missing_invoice_ids,
    ...
    SUM(CASE
        ...
        ELSE 0
    END) AS missing_date_keys,

    SUM(CASE
        WHEN track_id IS NULL THEN 1
        ELSE 0
    END) AS missing_track_ids,

    SUM(CASE
        WHEN employee_key IS NULL THEN 1
        ELSE 0
    END) AS missing_employee_keys,

    SUM(CASE
        WHEN quantity IS NULL OR quantity <= 0 THEN 1
        ELSE 0
    END) AS invalid_quantities,

    SUM(CASE
        WHEN unit_price IS NULL OR unit_price <= 0 THEN 1
        ELSE 0
    END) AS invalid_unit_prices,

    SUM(CASE
        WHEN line_amount IS NULL OR line_amount <= 0 THEN 1
        ELSE 0
    END) AS invalid_line_amounts
